...
async function capturarLead(dominio, score, ssl, reputacao, velocidade, plataforma) {
    try {
        ...
        const { data } = await _supabase.from('leads').insert([{
        ...
        if (data && data[0]) idLeadAtual = data[0].id;
...
async function finalizarSolicitacao() {
    ...
    try {
        ...
        if (leadRecente) {
            await _supabase.from('leads').update({ email: emailValue, score: "LEAD DIRETO" }).eq('id', leadRecente.id);
        } else {
...
async function iniciarDiagnostico() {
    ...
    try {
        ...
        const [dmarcData, totalAlertas] = await Promise.all([fetch(`https://dns.google/resolve?name=_dmarc.${dominio}&type=TXT`).then(r => r.json()).catch(() => ({})), checkReputation(dominio)]);
        const temDmarc = !!(dmarcData.Answer);
        let sslOk = false;
        try { const ctrl = new AbortController(); setTimeout(() => ctrl.abort(), 3500); await fetch(`https://${dominio}`, { mode: 'no-cors', signal: ctrl.signal }); sslOk = true; } catch { sslOk = false; }
        const duration = (Date.now() - start) / 1000;
        let cor = (sslOk && temDmarc && totalAlertas === 0) ? "#00FF00" : "#FF4444";
        let score = (sslOk && temDmarc && totalAlertas === 0) ? "A+" : "Crítico";
        const velStr = `Rápida (${duration.toFixed(1)}s)`;
        await capturarLead(dominio, score, sslOk ? "Ativo" : "Falha", totalAlertas, velStr, "Sistema Detectado");
        ...
        document.getElementById('btnPDF').onclick = () => gerarRelatorioPDF({ dominio, score, sslOk, totalAlertas, velStr, plataforma: "Sistema Detectado", temDmarc });
    } catch { resultArea.innerHTML = "Erro técnico."; }
}

function gerarRelatorioPDF(d) {
    const { jsPDF } = window.jspdf; const doc = new jsPDF(); const corTema = d.score === "A+" ? [0, 150, 0] : [180, 0, 0];
    doc.setFillColor(20, 20, 20); doc.rect(0, 0, 210, 45, 'F');
    try { doc.addImage("img/logo_shield_branco.png", "PNG", 15, 12, 50, 15); } catch { doc.setTextColor(255, 255, 255); doc.text("LONGO SHIELD", 15, 22); }
    ...
    doc.setTextColor(40, 40, 40); doc.setFontSize(14); doc.text(`Analise: ${d.dominio.toUpperCase()}`, 15, 75);
    ...
    doc.setFontSize(12); doc.text("POR QUE ESTES ITENS SAO CRITICOS?", 15, 145);
    doc.setFontSize(9); doc.text(["SSL: Garante que os dados nao sejam interceptados.", "DMARC: Impede que usem seu e-mail para golpes.", "Reputacao: Verifica malwares ou listas negras."], 15, 155);
    doc.setFontSize(11); doc.text("PARECER DO ESPECIALISTA:", 15, 180);
    doc.setFillColor(corTema[0], corTema[1], corTema[2]); doc.rect(15, 185, 180, 20, 'F');
    doc.setTextColor(255, 255, 255); doc.text(doc.splitTextToSize(d.score === "A+" ? "Ambiente em conformidade." : "RISCO DETECTADO: Recomendamos mitigacao imediata.", 170), 20, 197);
