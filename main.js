...
// Variável crucial para vincular o e-mail à análise técnica na mesma linha
...
// Salva a análise técnica e captura o ID para o posterior UPDATE
async function capturarLead(dominio, score, ssl, reputacao, velocidade, plataforma) {
    try {
        ...
        if (data && data[0]) {
            ...
            console.log("Sucesso! Linha técnica criada com ID:", idLeadAtual);
        }
    } catch (err) { console.error("Erro no insert inicial:", err); }
}

// ==========================================
// 3. FUNÇÕES DO MODAL E FINALIZAÇÃO
// ==========================================
...
// ATENÇÃO: Agora recebendo o 'event' corretamente para evitar erro de target
async function finalizarSolicitacao(event) {
    ...
    // Prevenção do erro TypeError que duplicava as linhas
    const btn = event ? event.target : document.querySelector('#modalEmail button');
    if (btn) {
    ...
    try {
        if (idLeadAtual) {
            // Tenta o UPDATE na linha técnica (Caminho perfeito)
            const { error } = await _supabase.from('leads')
                .update({ email: emailValue })
                .eq('id', idLeadAtual);
            
            if (error) throw error;
            console.log("E-mail vinculado ao ID:", idLeadAtual);
        } else {
            // Fallback: Busca a última linha técnica do domínio se o ID se perdeu
            const { data: busca } = await _supabase.from('leads')
                .select('id')
                .eq('dominio', dominio)
                .is('email', null)
            ...
            if (busca) {
                await _supabase.from('leads').update({ email: emailValue }).eq('id', busca.id);
            } else {
                throw new Error("Não achou linha técnica");
            }
        }
        ...
        // Só cria nova linha em caso de falha total de comunicação
        await _supabase.from('leads').insert([{ dominio, email: emailValue, score: "LEAD COMPLETO (SYNC)" }]);
        document.getElementById('modalEmail').style.display = 'none';
    } finally {
        if (btn) {
